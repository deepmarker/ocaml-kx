...
let date = testable pp_print_date Stdlib.( = )
...
let pack_unpack : type a. string -> a w -> a -> unit =
  ...
  match
    ( Angstrom.parse_bigstring ~consume:Prefix hdr serialized
    , Angstrom.parse_bigstring ~consume:Prefix (destruct w) payload )
  with
...
let pack_unpack_atom =
  Kx.
    ...
    ; pack_unpack "timestamp_now" (a timestamp) (Ptime_clock.now ())
...
let pack_unpack_table =
  Kx.
    ...
    ; pack_unpack "simple_sorted_bool1" (table1 ~sorted:true bool) ([| "a" |], [| true |])
    ; pack_unpack
        "simple_sorted_bool2"
  ...
  let w2 =
    union
      [ case
          ...
          (fun a -> Some a)
          (fun a -> a)
      ; case (conv (fun a -> a) (fun a -> a) (a Kx.int)) (fun a -> Some a) (fun a -> a)
      ]
  in
...
let unpack_buggy () =
  ...
  match
    ( Angstrom.parse_bigstring ~consume:Prefix hdr (Cstruct.to_bigarray test)
    , Angstrom.parse_bigstring
        ~consume:Prefix
        (destruct (a sym))
        (Cstruct.to_bigarray payload) )
  with
  | Error msg, _ | _, Error msg -> fail msg
  | Ok _hdr, Ok v ->
...
let test_various () =
  let sym = "\245a\000" in
  match Angstrom.parse_string ~consume:Prefix Kx.(destruct (a sym)) sym with
  | Ok _ -> ()
  | Error e -> failwith e
;;
...
let date ds =
  List.iter
    (fun d ->
      ...
      check date (Format.asprintf "%a" pp_print_date d) d d')
    ds
;;
...
let timestamp ts =
  List.iter
    (fun ts ->
      ...
      check ptime (Format.asprintf "%a" Ptime.pp ts) ts ts')
    ts
;;

let timestamp () = timestamp [ Ptime.v (100000, 0L); Ptime_clock.now () ]
...
let tests_kx =
  ...
  ; test_case
      ...
      `Quick
      (test_sync
         (`Hex "01000000210000007f0b0102000000610062000600020000000200000003000000")
  ...
  ; test_case
      "strange float vect"
      `Quick
      (test_sync (`Hex "010000001600000009000100000059f03b5dc8d78840") Kx.(v float))
  ; test_case
      "strange dict"
      `Quick
      (test_sync
